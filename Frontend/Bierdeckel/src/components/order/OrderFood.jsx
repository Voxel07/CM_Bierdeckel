...
function OrderFood({ products, newItems, handleStockChange, displayITems, handleDisplayItemChange }) {
    
  return (
    <Grid className='grid-container' container>
        {
            products?.length ? products.map(product => <Grid className='grid-item' item xl={3} lg={4} md={6} xs={12}>
                <InfoCard data = {product} userData={displayITems} handelChange={handleStockChange} />
                </Grid>) :null
        }
